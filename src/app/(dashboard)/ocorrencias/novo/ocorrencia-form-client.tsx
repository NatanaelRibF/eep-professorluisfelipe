...
export default function OcorrenciaFormClient({ classes, occurrenceTypes }: { classes: any[], occurrenceTypes: any[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div>
          ...
          <select 
            ...
            onChange={e => setOccurrenceTypeId(e.target.value)}
            required
          >
      ...
      <div>
        <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Descrição Detalhada dos Fatos</label>
        <Textarea 
      ...
      <div>
        <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Providência / Medida Tomada</label>
        <Textarea 
          placeholder="Ex: Notificação aos pais, encaminhamento à coordenação pedagógica, advertência..." 
          value={actionTaken} 
      ...
      <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 pt-4 border-t">
        ...
        <Button type="submit" disabled={submitting} className="bg-amber-600 hover:bg-amber-700 text-white font-bold h-11">
