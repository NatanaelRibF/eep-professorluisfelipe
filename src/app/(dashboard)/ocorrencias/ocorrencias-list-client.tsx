...
export default function OcorrenciasListClient({ classes, occurrenceTypes }: { classes: any[], occurrenceTypes: any[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-lg shadow-sm border grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
        <div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-slate-700 border-b">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold">Medida / Ação Tomada</th>
              ...
              <tbody className="divide-y">
                {data.map((item: any) => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-medium">{item.occurrenceType?.name}</div>
                      <span className={`inline-block px-2 py-1 mt-1 text-xs font-semibold rounded-full border ${getSeverityColor(item.occurrenceType?.severity)}`}>
                    ...
                    <td className="px-6 py-4 text-slate-600 max-w-xs truncate" title={item.actionTaken}>
                      {item.actionTaken || "-"}
                    </td>
